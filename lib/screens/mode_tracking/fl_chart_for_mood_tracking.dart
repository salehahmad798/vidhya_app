...
class FlChartForMoodTracking extends StatelessWidget {
  final List<MoodTrackingModel> trackingData;

  FlChartForMoodTracking({required this.trackingData});
              ...
              sideTitles: SideTitles(
                ...
                getTitlesWidget: (value, meta) {
                  if (value.toInt() >= 1 && value.toInt() <= trackingData.length) {
                    ...
                    return CText(
                      text: '${date.month}/${date.day}', // Show day and month
                      fontSize: 13,
                      color: Colors.green,
                    );
                  } else {
  ...
  List<FlSpot> _generateSpots(List<MoodTrackingModel> trackingData) {
    List<FlSpot> spots = [];
    for (int i = 0; i < trackingData.length; i++) {
      final item = trackingData[i];
      double x = i + 1;

      // Loop through all feelings of the day and add a spot for each feeling
      for (String feeling in item.feeling) {
        double y = _mapFeelingToValue(feeling); // Map each feeling to a value
